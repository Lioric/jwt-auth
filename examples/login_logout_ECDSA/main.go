...
func main() {
	authErr := jwt.New(&restrictedRoute, jwt.Options{
		...
		PrivateKeyLocation:    "keys/priv.pem", // `$ openssl ecparam -name prime256v1 -genkey -noout -out priv.pem`
		PublicKeyLocation:     "keys/pub.pem",  // `$ openssl ec -in priv.pem -pubout -out pub.pem`
